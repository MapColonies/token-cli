...
describe('generate-token', function () {
  beforeAll(async () => {
    await writeFile('/tmp/avi/generateTokenPrivateKey.jwk', JSON.stringify(privateKey), { encoding: 'utf-8' });
  });
  describe('Happy Path', function () {
    ...
    it('should return a signed token when origin is omitted', async function () {
      const { exitCode, stdout } = await executeCli(['generate-token', '-f', '/tmp/avi/generateTokenPrivateKey.jwk', '-c', 'avi']);
  ...
  describe('Bad Path', function () {
    it('should exit with error code if client is missing', async function () {
      const { exitCode, stderr } = await executeCli(['generate-token', '-f', '/tmp/avi/generateTokenPrivateKey.jwk', '-o', 'https://localhost:8080']);
    ...
    it('should exit with error code if file option is missing', async function () {
      const { exitCode, stderr } = await executeCli(['generate-token', '-c', 'avi', '-o', 'https://localhost:8080']);
      expect(exitCode).not.toEqual(0);
      const stdErrLines = stderr.split(EOL);
    ...
    it('should exit with error code if private key file doesn\t exist', async function () {
      const { exitCode, stderr } = await executeCli(['generate-token', '-f', '/dskdms.jwk', '-c', 'avi', '-o', 'https://localhost:8080']);
      expect(exitCode).not.toEqual(0);
    ...
    it('should exit if private key is not valid', async function () {
      ...
      const { exitCode, stderr } = await executeCli(['generate-token', '-f', '/tmp/avi/badKey.jwk', '-c', 'avi', '-o', 'https://localhost:8080']);
      expect(exitCode).not.toEqual(0);
